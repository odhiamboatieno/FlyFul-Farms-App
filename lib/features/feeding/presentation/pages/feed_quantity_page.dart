...
class _FeedQuantityPageState extends State<FeedQuantityPage> {
  ...
  Widget _buildDateEntry() {
    ...
    final date = feedingProvider.draftDate ?? DateTime.now();
    ...
    return GestureDetector(
      onTap: () async {
        ...
        if (picked != null) {
          feedingProvider.setDate(picked);
        }
      },
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(fmt.format(date), style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: AppColors.ink)),
        ])),
